...
public class BookServiceImpl implements BookService {
    ...
    public List<String> findTitlesByAgeRestriction(String input){
        ...
        List<Book> allByAgeRestriction = bookRepository.findAllByAgeRestriction(restriction);

        return mapBooksToTitles(allByAgeRestriction);

    }

    @Override
    public List<String> findGoldenBooksTitles() {
        List<Book> books = bookRepository.findAllByEditionTypeAndCopiesLessThan(EditionType.GOLD, 5000);
        return mapBooksToTitles(books);
    }

    @Override
    public List<String> findByPriceOutsideOf(BigDecimal lowerBound, BigDecimal upperBound) {
        List<Book> books = bookRepository.findByPriceLessThanOrPriceGreaterThan(lowerBound, upperBound);
        return mapBooksToTitles(books);
    }
    ...
    public List<String> findBooksNotReleasedIn(int year) {
        ...
        List<Book> result = bookRepository.findAllByReleaseDateBeforeOrReleaseDateAfter(yearStart, yearEnd);
    ...
    @Override
    public List<Book> findByAuthorLastNameStarting(String lastNameStartsWith) {
        List<Book> result = bookRepository.findByAuthorLastNameStartingWith(lastNameStartsWith);
        return result;
    }
